...
build ran on. They are retained unedited because they are a record of how the numbers were arrived
at rather than a specification of how to run the project; rewriting a path in a prompt that was
issued with that path would falsify what the agent was told. The path is not a credential — it names
a macOS username already implied by the GitHub handle and by `LICENSE`. The current run path is in
`README.md`.
