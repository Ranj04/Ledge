...
| 3 Volatile | `Episodes`, `Foresights`, `Cases` + the user's new message | every turn, or unknown | **no** |
...
- Use the repository's shared Python 3.12 virtualenv and its `python` and `pytest`
  executables. **Do not create another venv.**
- To add a package, activate the shared virtualenv and run `uv pip install <pkg>`, then name it in
  your final message so Fable adds it to `requirements.txt`.
