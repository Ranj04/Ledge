...
Open <http://localhost:8000>. No credentials needed. It runs against simulators, and the UI says on
screen whether the provider is a simulator.
...
The Context Assembler sorts retrieved memories by volatility, stable first and volatile last, and
marks cache breakpoints at the tier boundaries. Same memories, same information, same answer,
lower bill.
...
### A per-memory cost ledger

Every call records which memories were injected and what each one cost, at the rate its region of
the prompt was actually billed at. Rolled up, that is a per-memory monthly cost. An ablation harness
then replays calls with one memory removed and scores whether the answer changed. Memories that
cost money and change nothing become eviction candidates.
...
Machine-readable runs are in [`results/`](results/), simulator runs only; the 2026-08-07 live run
is not among them.
...
live 42.9% headline was measured on 2026-08-07 against the plain bullet format and has not been
re-measured live, because there is no `OPENAI_API_KEY` on the build machine. The simulator was
run against all four formats on the same corpus and the same scripted conversations, 3 x 4 runs
each; the markup is the only difference between the columns.
...
| reduction, mean (naive to tiered) | 52.35% | 52.81% | 52.26% | 52.72% | +0.37 pt |
...
Stage 4 recovers 97 to 98% of that rise. What is left over bullets is one thing: a 65-token
...
`cached_tokens / input_tokens`, and `input_tokens` is the total prompt: cached, written, and the
current turn, which can never be cached. Dividing by the cacheable region instead would produce a
larger number; this is the conservative framing and `/api/chat` and the session totals use it
identically.
...
real model the two modes sample independently, so replies differ in length by ~20%, and folding
that into a caching number would be measuring sampling noise.

---

## Why the numbers are real

Two kinds of measurement appear in this document, each with its own provenance:

- The headline (42.9% mean input-side reduction, 47.9% `tiered` hit rate, 0.0% `naive`) is
...
- The Stage 1 / 2 / 3 / 4 comparison table ("What the provenance delimiter did to the numbers") is
...
Two measurement bugs each produced a plausible, wrong number with no error anywhere:
...
verdicts (`DECISIONS.md` D10). The UI shows the provider state on screen at all times.
...
guessing. Writes bill at 1.25x and land on tokens not served from cache, which is the naive
...
`naive` mode puts memories at the front of the prompt in relevance order with no breakpoints, which
is how a memory-augmented agent is built when nobody has thought about caching. It is production
code, not a strawman. Both modes retrieve the same memories and put the same information in front
of the model; two tests fail the build if that stops being true. The argument in full is
`DECISIONS.md` D6.
...
- [`DECISIONS.md`](DECISIONS.md): every ambiguous call and why.
- [`BLOCKERS.md`](BLOCKERS.md): what could not be verified without credentials, corrections, and
  unresolved work.
- [`results/`](results/): machine-readable measurement artifacts and provenance.
- [`docs/history/`](docs/history/): dated records from the build and the event it was built for,
  not maintained. The event-day checklist, the 3-minute demo script, the cross-agent handoff log.
...
| 1 Durable | `Profiles` | weeks to months | yes |
...
`/api/status` rather than keeping a copy. `Foresights` and `Cases` sit in tier 3 as the safe
default: calling a volatile type stable destroys the cache for every tier behind it, while
calling a stable type volatile only forgoes some savings (`DECISIONS.md` D24).

Order is by measured prefix stability, not tier number: tier 0, then tier 1, then the conversation
history, then tiers 2 and 3. Conversation history is append-only so its prefix never changes; a
top-k retrieval reshuffles every question. A churning block poisons every stable block behind it,
so `Facts` ride behind the history (`DECISIONS.md` D17).
...
"explicit"` disables the automatic longest-prefix match, and you pay a 1.25x write surcharge for
...
It is the first backend on which "the stores agree" is a test rather than a docstring
